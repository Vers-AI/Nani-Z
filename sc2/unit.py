...
class Unit:
    ...
    def type_id(self) -> UnitTypeId:
        ...
        return self.class_cache.retrieve_and_set(
            unit_type, lambda: UnitTypeId(unit_type)
        )
    ...
    def calculate_speed(self, upgrades: Set[UpgradeId] = None) -> float:
        ...
        if (
            ...
            # Off creep upgrades
                ...
                upgrade_id2: Optional[UpgradeId] = OFF_CREEP_SPEED_UPGRADE_DICT.get(
                    unit_type, None
                )
                if upgrade_id2:
    ...
    def build_gas(
        ...
        return self(
            self._bot_object.game_data.units[
    ...
    def research(
        ...
        """
        return self(
            self._bot_object.game_data.upgrades[
                upgrade.value
    ...
    def warp_in(
        ...
        :param can_afford_check:
        """
        normal_creation_ability = self._bot_object.game_data.units[
            unit.value
        ].creation_ability.id
        return self(
    ...
    def attack(
        self, target: Union[Unit, Point2], queue: bool = False
    ) -> Union[UnitCommand, bool]:
    ...
    def move_org(
        self, position: Union[Unit, Point2], queue: bool = False
    ) -> Union[UnitCommand, bool]:
    ...
    # move_check
    def move(
        self, position: Union[Unit, Point2], distance: float = 1.0, queue: bool = False
    ) -> Union[UnitCommand, bool]:
        ...
        if self.orders and self.orders[0].ability.id in {AbilityId.MOVE}:
            # target is an Unit
            if (
                isinstance(self.orders[0].target, int)
                and isinstance(position, Unit)
                and self.orders[0].target == position.position
            ):
                return True
            ...
            if not isinstance(self.orders[0].target, int):
                ...
                if (
                    ...
                    or position.distance_to(target_position) < distance
                ):
                    return True
