...
class Ramp:
    ...
    def upper2_for_ramp_wall(self) -> FrozenSet[Point2]:
        ...
        return frozenset(
            heapq.nlargest(
                2, self.upper, key=lambda x: x.distance_to_point2(self.bottom_center)
            )
        )
    ...
    def top_center(self) -> Point2:
        ...
        pos = Point2(
            (
                sum(p.x for p in self.upper) / length,
                sum(p.y for p in self.upper) / length,
            )
        )
        return pos
    ...
    def bottom_center(self) -> Point2:
        ...
        pos = Point2(
            (
                sum(p.x for p in self.lower) / length,
                sum(p.y for p in self.lower) / length,
            )
        )
        return pos
...
class GameInfo:
    def __init__(self, proto):
        ...
        self.pathing_grid: PixelMap = PixelMap(
            self._proto.start_raw.pathing_grid, in_bits=True
        )
        ...
        self.placement_grid: PixelMap = PixelMap(
            self._proto.start_raw.placement_grid, in_bits=True
        )
    ...
    def _find_groups(
        self, points: FrozenSet[Point2], minimum_points_per_group: int = 8
    ) -> Iterable[FrozenSet[Point2]]:
        """
